...
public class Login extends AppCompatActivity {
    TextInputEditText name, password, edtEmail, edtPass;
    Button btnLogin;
 ...
 // txtMessage.setText(message);
        ...
        signInButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                Log.d("asd","tyu"+Uname+pwd);

                ApiInterface apiInterface=ApiClient.getClient().create(ApiInterface.class);
                Call<Model> modelCall = apiInterface.login("get/GoogleLogin",Uname,pwd,BuildConfig.VERSION_NAME,deviceToken);
                modelCall.enqueue(new Callback<Model>() {
                    @Override
                    public void onResponse(Call<Model> call, Response<Model> response) {
                        Log.d("asd","tyu"+response.toString());
                        ...
                        try {
                            if (response.isSuccessful()) {
                                ...
                                if (response.body().getSuccess() == true) {
                                    ...
                                    try {
                                        Gson gson = new Gson();
                                        assignLoginData(response.body(), RC_SIGN_IN);
...
*/
    ...
    public void loginn(int requestCode) {
        String Uname = name.getText().toString();
        String pwd = password.getText().toString();
    ...
    void assignLoginData(Model response, int requestCode) {
        try {
            ...
            if (response.getData().get(0).getLoginType() != null &&
                ...
                shared_common_pref.save(Constants.TEMP_DISTRIBUTOR_ID, response.getData().get(0).getDistributor_Code());
